...
export class SilverService {
 ...
 async  getPriceFromKitco(): Promise<string> {
  ...
  try {
    ...
    const selector = 'main div.flex > div:nth-child(2) div.text-right.font-medium';
    await page.waitForSelector(selector);
...
async getZiotoSilverBars(): Promise<string> {
  ...
  try {
    ...
    const url = 'https://zioto.gold/index.php?route=product/category&language=fa-ir&path=60';
    await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 60000 });
    ...
    for (let i = 0; i < xpaths.length; i++) {
      ...
      const cleaned = this.toEnglishDigits(price.replace(/تومان/g, '').trim());
      const numericPrice = cleaned.replace(/[^0-9]/g, '');

      results.push(`${weights[i]} : ${this.formatNumber(Number(numericPrice))}`);
    }
